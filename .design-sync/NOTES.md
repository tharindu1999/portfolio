...
- `Magnetic.d.ts` extracts as a permissive fallback (`{ [key: string]: unknown }`) — the ts-morph prop extraction can't resolve the framer-motion `motion.div` props. Consequence: adding/removing props on `Magnetic` (e.g. the `className` prop added 2026-07-05) does NOT change its emitted `.d.ts` or its diff, so such changes ride the bundle/styling upload rather than a per-component one. The design agent gets no typed contract for Magnetic (accepts any prop) — acceptable for a thin wrapper. If a real contract is ever needed, set `cfg.dtsPropsFor.Magnetic`.
- A big App.tsx change (like the Portfolio redesign) recompiles the Tailwind subset, so `styleSha` changes and every re-sync re-uploads bundle+styling even when no component render moved. Expected, not a bug.
